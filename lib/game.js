...
function preload() {
    ...
    game.load.audio('audio', 'assets/audio/Space.ogg');
    game.load.audio('launch', 'assets/audio/Launch.ogg');
}
...
function teleportPlayer(player){
 //   if(parts==0){ uncomment if to only allow teleportation when all parts have been found.
        player.body.x = 1750;
        player.body.y = 0;
   // }
}
...
//add broken spaceship
function createBrokenShip(x, y){
    brokenShip = game.add.group();
    //enable collision
    brokenShip.enableBody = true;
    //add the ship at x,y coordinates with the picture of the broken ship
    ship = brokenShip.create(x, y, 'brokenShip');
}

//add catfood to the level
function createItems(){
    //create group and add body -> can collide
    items = game.add.group();
    items.enableBody = true;

    //create catfood arch at (x, y, how many*2, spacing)
...
function createSpaceship(){
    ...
    //add part at (x, y, picture) and show picture
...
//add cookie at (x, y, picture) with the cookie picture.
function addSpeedBoost(x, y){
    cookie = speedBoosts.create(x, y, 'cookie');
}

//generating the arch of catfood. 
...
function removeBubble(){
  ...
  //      game.time.events.add(Phaser.Timer.SECOND * 1, ship.loadTexture('spaceship'), this);
        ...
        game.time.events.add(Phaser.Timer.SECOND * 12, ship.animations.play('liftof'), this);
...
function collectSpeedBoost(player, item){
    ...
    game.time.events.add(Phaser.Timer.SECOND * 4, normalSpeed, this);
    speed *= 3;
    addToScore(500); 
}
